        ...
        private void SetBeiShu()
        {
            for (int i = 0; i < custom.Count; i++)
            {
                ...
                for (int j = 0; j < YGBH_DataConfig.BeiLVTable.Count; j++)
                {
                    ...
                    {
                        count = count + 1;
                        custom[i].GetChild(count - 1).GetComponent<Text>().text = (YGBH_DataConfig.BeiLVTable.Count - j + 1) + "  " + GetCustomK(YGBH_DataConfig.BeiLVTable[j][i] * YGBHEntry.Instance.GameData.CurrentChip);
                    }
                }
            }
        }
