...
namespace Hotfix.LTBY
{
    public class LTBY_MenuView : LTBY_ViewBase
    {
        ...
        private void ShowGameUi(bool flag)
        {
            ...
            if (flag)
            {
                Tween tween = transform.DOBlendableLocalMoveBy(new Vector3(-GameViewConfig.ShowGameUi.Dis, 0), GameViewConfig.ShowGameUi.Time).SetEase(Ease.OutBack).OnComplete(() =>
                  {
            ...
            {
                Tween tween = transform.DOBlendableLocalMoveBy(new Vector3(GameViewConfig.ShowGameUi.Dis, 0), GameViewConfig.ShowGameUi.Time).SetEase(Ease.InBack).OnComplete(() =>
                {
                    StopAction(this.showUiActionKey);
                    this.showUiActionKey = 0;
                });
        ...
        private void OpenView()
        {
            ...
            Tween tween = menuFrame?.DOScale(new Vector3(1, 1), 0.2f).OnComplete(() =>
              {
                  this.actionPlaying = false;
              });
            RunAction(tween);
        ...
        private void CloseView()
        {
            ...
            Tween tween = menuFrame?.DOScale(new Vector3(0, 0), 0.2f).OnComplete(() =>
            {
