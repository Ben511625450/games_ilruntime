    ...
    public class YGBH_Result : ILHotfixEntity
    {
        ...
        private class NormalWinState : State<YGBH_Result>
        {
            ...
            public override void OnEnter()
            {
                ...
                owner.winNormalEffect.DOScale(Vector3.one, 0.3f).OnComplete(()=> 
                {
                    ...
                    {
                        owner.winNormalNum.text = ToolHelper.ShowRichText(goal);
        ...
        private class BigWinState : State<YGBH_Result>
        {
            ...
            public override void OnEnter()
            {
                ...
                owner.goodEffect.DOScale(Vector3.one, 0.3f).OnComplete(()=> 
                {
                    ToolHelper.RunGoal(0, YGBHEntry.Instance.GameData.ResultData.WinScore, YGBH_DataConfig.winBigGoldChangeRate, delegate (string goal)
                    {
                        owner.goodProgress.value = (float)(float.Parse(goal) / YGBHEntry.Instance.GameData.ResultData.WinScore);
                        owner.winGoodNum.text = ToolHelper.ShowRichText(goal);
                        //owner.WinNum.text = ToolHelper.ShowRichText(goal);
        ...
        private class SuperWinState : State<YGBH_Result>
        {
            ...
            public override void OnEnter()
            {
                ...
                owner.winPerfectNum.text = ToolHelper.ShowRichText("0");
                ...
                owner.perfectEffect.DOScale(Vector3.one, 0.3f).OnComplete(()=> 
                {
                    ToolHelper.RunGoal(0, YGBHEntry.Instance.GameData.ResultData.WinScore, YGBH_DataConfig.winBigGoldChangeRate, delegate (string goal)
                    {
                        owner.prefectProgress.value = (float.Parse(goal) / YGBHEntry.Instance.GameData.ResultData.WinScore);
                        owner.winPerfectNum.text = ToolHelper.ShowRichText(goal);
                    }).OnComplete(delegate ()
                    {
                        isStart = true;
                    });
        ...
        private class UltraWinState : State<YGBH_Result>
        {
            ...
            public override void OnEnter()
            {
                ...
                owner.winBigNum.text = ToolHelper.ShowRichText("0");
                ...
                owner.bigEffect.DOScale(Vector3.one, 0.3f).SetEase(DG.Tweening.Ease.Linear).OnComplete(()=> 
                {
                    ToolHelper.RunGoal(0, YGBHEntry.Instance.GameData.ResultData.WinScore, YGBH_DataConfig.winBigGoldChangeRate, delegate (string goal)
                    {
                        float currentRunGold = (float)Mathf.Ceil(float.Parse(goal));
                        if (currentRunGold >= YGBHEntry.Instance.GameData.ResultData.WinScore)
                        {
                            currentRunGold = YGBHEntry.Instance.GameData.ResultData.WinScore;
                        }
                        float remain = (currentRunGold - index * (YGBHEntry.Instance.GameData.ResultData.WinScore / 3)) / (YGBHEntry.Instance.GameData.ResultData.WinScore / 3);
                        ...
                        owner.winBigNum.text = ToolHelper.ShowRichText(goal);
                    }).OnComplete(delegate ()
                    {
                        isStart = true;
                    });
        ...
        private class ShowCSJLResultEffect : State<YGBH_Result>
        {
            ...
            public override void OnEnter()
            {
                ...
                owner.winSmallEffectNum.text = ToolHelper.ShowRichText("0");
                ...
                owner.smallEffect.DOScale(Vector3.one, 0.3f).OnComplete(() =>
                {
                    ...
                    {
                        int currentRunGold = (int)Mathf.Ceil(float.Parse(goal));
                        owner.winSmallEffectNum.text = ToolHelper.ShowRichText(goal);
