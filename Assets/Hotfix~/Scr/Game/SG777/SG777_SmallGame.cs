    ...
    public class SG777_SmallGame : ILHotfixEntity
    {
        ...
        public class EnterBonusState : State<SG777_SmallGame>
        {
            ...
            public override void OnEnter()
            {
                ...
                if (SG777Entry.Instance.GameData.SmallGameData!=null && SG777Entry.Instance.GameData.SmallGameData.nSmallGameCount>0)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < SG777Entry.Instance.GameData.SceneData.nSmallGameData.Count; i++)
                        {
                            ...
                            if (item[i].isOpen>=1)
                            {
                                owner.ItemCont.GetChild(item[i].isOpen).FindChildDepth<Button>("button").interactable = false;
                                owner.ItemCont.GetChild(item[i].isOpen).FindChildDepth("moneycont").gameObject.SetActive(true);
                                owner.ItemCont.GetChild(item[i].isOpen).FindChildDepth<TextMeshProUGUI>("moneycont").text =ToolHelper.ShowRichText(item[i].nBellGold);
                                owner.NoOpenList.Remove(item[i].isOpen);
                            }
                        }
                    }
                }
                owner.LastCont.text = owner.SmallCount.ToString();
                owner.WinNum.text = owner.allWinSMGold.ToString();
        ...
        public class BonusState : State<SG777_SmallGame>
        {
            ...
            public override void OnEnter()
            {
                timer = 0f;
                isEnter = false;
                Transform item = owner.ItemCont.GetChild(SG777Entry.Instance.GameData.SmallGameData.nStopIndex);
                ...
                { 
                    ...
                    ToolHelper.RunGoal(0, SG777Entry.Instance.GameData.SmallGameData.nWinGold,1.5f, delegate (string goal)
                    {
                        itemText.text =ToolHelper.ShowRichText(goal);
                    ...
                    }).OnComplete(delegate ()
                    {
                        ...
                        owner.WinNum.text = $"{owner.allWinSMGold}";
        ...
        private class SmallOverState : State<SG777_SmallGame>
        {
            ...
            public override void OnEnter()
            {
                ...
                ToolHelper.RunGoal(0, owner.allWinSMGold, SG777_DataConfig.winBigGoldChangeRate, delegate (string goal)
                {
