            ...
            IEnumerator StartRollInfo()
            {
                ...
                for (int i = 0; i < CMLHJEntry.Instance.GameData.MaryData.nByPrizeImg.Count; i++)
                {
                    if (CMLHJEntry.Instance.GameData.MaryData.nByPrizeImg[i] == owner.C_MARY_IMG[CMLHJEntry.Instance.GameData.MaryData.nStopIndex])
                    {
                        ...
                        obj.FindChildDepth<TextMeshProUGUI>("Num/Text").text = ToolHelper.ShowRichText(data);
                        ...
                        while (data > 0)
                        {
                            ...
                            CMLHJEntry.Instance.WinInfoNum.text = ToolHelper.ShowRichText(allgold);
                            yield return new WaitForSeconds(0.2f);
                        }
                    }
                }

                CMLHJEntry.Instance.WinInfoNum.text = ToolHelper.ShowRichText(CMLHJEntry.Instance.GameData.MaryData.nWinGold);
                yield return new WaitForSeconds(0.5f);
