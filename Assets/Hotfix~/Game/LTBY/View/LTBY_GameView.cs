    ...
    public class LTBY_GameView : LTBY_ViewBase
    {
        ...
        public void CreateAddUserScoreItem(int _chairId, long score, bool serial = false)
        {
            ...
            Transform coin = this.coinList[_chairId];
            {
                ...
                key.Append(coin?.DOScale(1.6f, 0.1f).SetEase(Ease.Linear));
                key.Append(coin?.DOScale(1f, 0.1f).SetEase(Ease.Linear));
                RunAction(key);
            }
            ...
            roller.Init();
            roller.text = score.ToString();
            item.GetComponent<TextMeshProUGUI>().text = ToolHelper.ShowRichText($"+{score}");
            ...
            if (serial)
            {
                ...
                key.Append(_key1);
                _key1.Append(item.GetComponent<TextMeshProUGUI>().DOFade(255f / 255, 0.01f).SetEase(Ease.Linear));
                _key1.Append(item.DOBlendableLocalMoveBy(new Vector3(0, offsetY, 0), 0.7f).SetEase(Ease.OutSine));
                _key1.Append(item.GetComponent<TextMeshProUGUI>().DOFade(0f / 255, 0.2f).SetEase(Ease.Linear));
                Sequence _key2 = DOTween.Sequence();
                key.Insert(0, _key2);
                _key2.Append(item?.DOScale(1, 0.01f).SetDelay(0.1f));
                _key2.Append(item?.DOScale(new Vector3(1.1f, 1.1f), 0.2f));
                _key2.Append(item?.DOScale(new Vector3(1, 1), 0.2f).OnComplete(() => LTBY_PoolManager.Instance.RemoveUiItem(_prefabName, item)));
                RunAction(key);
            }
            else
            {
                ...
                key.OnComplete(() => LTBY_PoolManager.Instance.RemoveUiItem(_prefabName, item));
                RunAction(key);
            }
        }
        ...
        private void ShowGameUi(bool flag)
        {
            ...
            if (flag)
            {
                ...
                }, 0, 100, GameViewConfig.ShowGameUi.Time).SetEase(Ease.Linear));
            }
            else
            {
                showUiActionKey = RunAction(btnShop
