...
namespace Hotfix.LTBY
{
    ...
    public class BatteryClass : IILManager
    {
        ...
        public virtual void SetRatio(int _ratio)
        {
            ...
            sequence.Append(num?.transform?.DOScale(new Vector3(0.8f + scale.x, 0.8f + scale.y), 0.1f));
            sequence.Append(num?.transform?.DOScale(new Vector3(-0.8f + scale.x, -0.8f + scale.y) + scale, 0.1f));
            ...
            if (LTBY_GameView.GameInstance.IsSelf(_chairId))
            {
                ...
                if (this.shootInterval <= 0 && LTBY_BatteryManager.Instance.GetCanShoot())
                {
                    if (this.multiShoot)
                    {
                        ...
                        //    bullets = bulletInfo,
                        //});
                        // ToolHelper.DelayRun(0.3f, delegate ()
                        // {
                        // for (int i = 0; i < bulletInfo.Count; i++)
                        // {
                        //     fish.OnHit(this.chairId, bulletInfo[i]);
                        // }
                        // });
                    }
                    else
                    {
                        ...
                        {
                            //fish.OnHit(this.chairId, bulletId);
                        });
                    }

                    LTBY_GameView.GameInstance.MinusScore(this.chairId, this.ratio * bulletCount);
                    LTBY_DataReport.Instance.Cost("子弹", this.ratio * bulletCount);
                }
        ...
        {
            if (lockFish == null || lockFish.isDead || !lockFish.inScreen)
            {
                lockFish = null;
    ...
    public class ElectricBatteryClass : BatteryClass
    {
        ...
        public void OnCreate(int _chairId, int _ratio, Transform _parent)
        {
            ...
            if (LTBY_GameView.GameInstance.IsSelf(this.chairId))
            {
                ...
                Sequence sequence = DOTween.Sequence();
                sequence.Append(this.battery.DOBlendableScaleBy(new Vector3(0.5f, 0.5f, 1), 0.2f).SetEase(Ease.Linear));
                sequence.Append(
                    this.battery.DOBlendableScaleBy(new Vector3(-0.5f, -0.5f, 1), 0.2f).SetEase(Ease.Linear));
                sequence.OnComplete(delegate()
                {
                    this.shootPos.localScale = new Vector3(0, 2, 2);
                    Sequence _sequence = DOTween.Sequence();
                    _sequence.Append(shootPos?.DOScale(2, 0.3f).SetEase(Ease.Linear));
                    Sequence _sequence1 = DOTween.Sequence();
                    _sequence.Append(_sequence1);
                    _sequence1.Append(shootPos?.GetComponent<Image>().DOFade(30f / 255, 1).SetEase(Ease.Linear));
                    _sequence1.Append(shootPos?.GetComponent<Image>().DOFade(255f / 255, 1).SetEase(Ease.Linear));
                    _sequence1.SetLoops(10);
        ...
        protected override void InitTip()
        {
            ...
            Sequence sequence = DOTween.Sequence();
            sequence.Append(tipImage.DOBlendableScaleBy(new Vector3(1, 1, 1), 0.1f).SetEase(Ease.Linear));
            sequence.Append(tipImage.DOBlendableScaleBy(new Vector3(-1, -1, 1), 0.2f).SetEase(Ease.Linear));
            int key = LTBY_Extend.Instance.RunAction(sequence);
            actionKey.Add(key);
        ...
        {
            base.OnUpdate();
            if (LTBY_GameView.GameInstance.IsPlayerRunInBackground(this.chairId)) return;
    ...
    public class MissileBatteryClass : BatteryClass
    {
        ...
        public void OnCreate(int _chairId, int _propId, Transform _parent)
        {
            ...
            {
                Sequence sequence = DOTween.Sequence();
                sequence.Append(battery.DOBlendableScaleBy(new Vector3(0.5f, 0.5f, 1), 0.2f).SetEase(Ease.Linear));
                sequence.Append(battery.DOBlendableScaleBy(new Vector3(-0.5f, -0.5f, 1), 0.2f).SetEase(Ease.Linear));
                int key = LTBY_Extend.Instance.RunAction(sequence);
                actionKey.Add(key);
                LTBY_GameView.GameInstance.SetAdjustRatioEnable(this.chairId, false);
        ...
        protected override void InitTip()
        {
            ...
            Sequence sequence = DOTween.Sequence();
            sequence.Append(battery.DOBlendableScaleBy(new Vector3(1f, 1f, 1), 0.1f).SetEase(Ease.Linear));
            sequence.Append(battery.DOBlendableScaleBy(new Vector3(-1f, -1f, 1), 0.2f).SetEase(Ease.Linear));
            int key = LTBY_Extend.Instance.RunAction(sequence);
            actionKey.Add(key);
    ...
    public class FreeBatteryClass : BatteryClass
    {
        ...
        public void OnCreate(int _chairId, int _ratio, int _mul, float _time, Transform _parent, bool begin)
        {
            ...
            this.status.FindChildDepth<Text>("Ratio").text = LTBY_BatteryManager.Instance.ChangeUnit(this.ratio);
        ...
        public void AddMul(int _mul)
        {
            ...
            sequence.Append(mulNode?.transform?.DOBlendableScaleBy(new Vector3(0.8f, 0.8f, 1), 0.1f)
                .SetEase(Ease.Linear));
            sequence.Append(mulNode?.transform?.DOScale(new Vector3(1f, 1f, 1), 0.1f).SetEase(Ease.Linear));
            int key = LTBY_Extend.Instance.RunAction(sequence);
            actionKey.Add(key);
        }
        ...
        public void AddTime(float _time)
        {
            ...
            Sequence sequence = DOTween.Sequence();
            sequence.Append(
                timeNode?.transform?.DOBlendableScaleBy(new Vector3(0.8f, 0.8f, 1), 0.1f).SetEase(Ease.Linear));
            sequence.Append(timeNode?.transform?.DOScale(new Vector3(1f, 1f, 1), 0.1f).SetEase(Ease.Linear));
            int key = LTBY_Extend.Instance.RunAction(sequence);
            actionKey.Add(key);
        }
        ...
        private class SelfState : State<FreeBatteryClass>
        {
            ...
            {
                base.Update();
                if (LTBY_GameView.GameInstance.IsPlayerRunInBackground(owner.chairId)) return;
