...
namespace Hotfix.LTBY
{
    ...
    public class LTBY_ResourceManager : SingletonNew<LTBY_ResourceManager>
    {
        ...
        public UnityEngine.Transform LoadPrefab(string bundleName, string prefabName,Transform parent=null, string name = null, string layerName = null)
        {
            ...
            if (go == null)
            {
                go = /*Util.isEditor ? Util.LoadPrefabEditor($"{editorPrefabPath}/{prefabName}.prefab") : */Util.LoadAsset(bundleName, prefabName);
            }
            if (go == null) return null;
            GameObject _go = GameObject.Instantiate<GameObject>(go);
            ...
            if (!cacheBundleDic.ContainsKey(prefabName))
            {
                cacheBundleDic.Add(prefabName, go);
            }
        ...
        public Sprite LoadAssetSprite(string bundleName, string assetName)
        {
            if (imgParent == null)
            {
                imgParent = /*Util.isEditor ? Util.LoadPrefabEditor($"{editorPrefabPath}/LTBY_FishImgs.prefab") :*/ Util.LoadAsset(bundleName, $"LTBY_FishImgs");
            }
        ...
        public AudioClip LoadAudioClip(string assetName)
        {
            if (SoundParent == null)
            {
                SoundParent = /*Util.isEditor ? Util.LoadPrefabEditor($"{editorSoundPath}/LTBY_Audio.prefab") :*/ Util.LoadAsset(soundBundleName, $"LTBY_Audio");
            }
            if (SoundParent == null)
            {
