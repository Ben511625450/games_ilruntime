    ...
    public class LTBY_SelectBattery : LTBY_Messagebox
    {
        ...
        public void InitContent(List<GunInfoData> gunInfo, int lastLevel)
        {
            ...
            for (int i = 0; i < gunInfo.Count; i++)
            {
                ...
                int _level = gunInfo[i].gun_level;
        ...
        public override void EnterAction()
        {
            ...
            Tween tween = board?.transform?.DOScale(new Vector3(1, 1, 1), 0.3f).SetEase(Ease.OutBack).OnComplete(() =>
            {
                actionPlaying = false;
            });
            RunAction(tween);
        ...
        public override void ExitAction()
        {
            if (actionPlaying) return;
            actionPlaying = true;
            Tween tween = board?.transform?.DOScale(new Vector3(0, 0, 0), 0.3f).SetEase(Ease.InBack).OnComplete(() =>
            {
                SetActive(false);
                actionPlaying = false;
                checkTime = false;
