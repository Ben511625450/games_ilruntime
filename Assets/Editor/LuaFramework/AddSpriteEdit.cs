...
public class AddSpriteEdit : EditorWindow, IActiveBuildTargetChanged
{
    ...
    public static void SaveHttpConfig()
    {
        ...
        string str = Util.EncryptDES(str1, "89219417");
        Debug.LogError(str);
...
//        MD5File();
    ...
    public static void BuildSceneAndPrefab()
    {
        ...
        string[] pathpools=Directory.GetFiles($"{Application.streamingAssetsPath}/module02/Pool", "*.unity3d", SearchOption.AllDirectories);
    ...
    public static void ESSingleAssetBundle()
    {
        ...
        for (int j = 0; j < platformlist.Count; j++)
        {
            ...
            for (int k = 0; k < bundlenames.Count; k++)
            {
                ...
                {
                    string filename = Path.GetFileNameWithoutExtension(files[i]);
                    string filePath = files[i].Replace(path, newPath);
    ...
    {
        AddSpriteEdit myWindow = (AddSpriteEdit)EditorWindow.GetWindow(typeof(AddSpriteEdit), false, "打包预制件", true);//创建窗口
        myWindow.Show();//展示
    }
