...
function SESXEntry.InitPanel()
    ...
    for i = 1, self.SceneData.chipNum do
        ...
        num.text = tostring(self.SceneData.chipList[i]);
        ...
        tog.onValueChanged:AddListener(function(value)
            if value then
                ...
                self.ChipNum.text = tostring(bet * SESX_DataConfig.ALLLINECOUNT);
                self.closeChipBtn.gameObject:SetActive(false);
            end
        end);
    end

    self.ChipNum.text = tostring(self.CurrentChip * SESX_DataConfig.ALLLINECOUNT);
    self.WinNum.text = tostring(0);
