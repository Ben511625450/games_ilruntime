...
function SHT_Result.ShowResult()
    ...
    SHTEntry.WinNum.text = tostring(SHTEntry.ResultData.WinScore);
    self.isClickShouFen = false;
...
function SHT_Result.ShowCSJLResult()
    ...
    if self.isShowCSJLResult then
        ...
        SHTEntry.freeOverTime.text=SHTEntry.ShowText(string.format("%.1f",(self.hideFreeTime-self.timer)))
...
function SHT_Result.ShowFreeEffect()
    ...
    self.showResultCallback = function()
        ...
        SHTEntry.FreeCountText.text=SHTEntry.ShowText(tostring(SHTEntry.ResultData.FreeCount))
        SHTEntry.TopFreeCountText.text=SHTEntry.ShowText(tostring(SHTEntry.ResultData.FreeCount)) 
        SHTEntry.freedownTime.text= SHTEntry.ShowText(self.hideFreeTime)
...
function SHT_Result.HideFree()
    ...
    if self.isHideFree then
        self.timer = self.timer + Time.deltaTime;
        SHTEntry.freedownTime.text=SHTEntry.ShowText(string.format("%.1f",(self.hideFreeTime-self.timer)))
        if self.timer >= self.hideFreeTime then
            SHT_Audio.PlayBGM(SHT_Audio.SoundList.FreeBgm);
            SHTEntry.freedownTime.text=SHTEntry.ShowText(0)
            --等待退出动画时长
