...
function BaccaraPanel.FindComponent()
    ...
    for i = 1, 5 do
        self.ShowChipScore.transform:GetChild(i - 1):Find("Text"):GetComponent("Text").text = "限红：0"
    end
    -- 开始发牌
...
function BaccaraPanel.SceneInfoMethod(wMain, wSubID, buffer, wSize)
    ...
    self.PokerNumText.text = "剩余牌数：" .. CardCount
    ...
    self.HuishouNumText.text = "当前 " .. buffer:ReadUInt32() .. " 轮 " .. buffer:ReadUInt32() .. " 局"
    ...
    if (iMiChipLimit > 0) then
        comText.text = comText.text .. iMiChipLimit .. "万才能眯牌";
    else
        comText.text = "";
    end
    self.SetOtherPlayer();
...
function BaccaraPanel.GameQuitMethod(wMain, wSubID, buffer, wSize)
    ...
    elseif BaccaraMH.BankerInfoTable._01wChair == BaccaraMH.mySelfInfo.wchair then
        self.ShowMessageTishiPanel("您现在正在当庄，强行退出游戏的话会“惩罚性扣除500万金币”，是否继续？")
    else
        -- Network.OnException("当前正在游戏，是否需要退出？");
        self.ShowMessageTishiPanel("当前正在游戏，是否需要退出？")
    end
end
...
function BaccaraPanel.BetAreaMethod(isFirst)
    ...
    for i = 1, #limitMaxChipTable do
        self.ShowChipScore.transform:GetChild(i - 1):Find("Text"):GetComponent("Text").text = "限红：" .. limitMaxChipTable[i]
    end
...
function BaccaraPanel.GameOver()
    ...
    if toInt64(BaccaraMH.mySelfInfo.gold) < toInt64(ChooseChipTable[1]) then
        ...
        HallScenPanel.NetException("当前金币不足,请充值!", gameSocketNumber.GameSocket);
        for i = 1, #C_CHIP_AREA_BTN do
            C_CHIP_AREA_BTN[i]:GetComponent("Button").interactable = true;
        end
    end ;
