...
function JSYCEntry.CloseGameCall()
    if self.isFreeGame then
        MessageBox.CreatGeneralTipsPanel("特殊游戏中无法退出游戏")
        return
    end
...
function JSYCEntry.SetSoundCall()
    ...
    JSYC_Audio.pool.mute = not AllSetGameInfo._5IsPlayAudio;
end
...
function JSYCEntry.AutoStartCall()
    ...
    if self.currentAutoCount > 1000 then
        self.autoText.text = JSYCEntry.ShowText("w");
    else
        self.autoText.text =JSYCEntry.ShowText(self.currentAutoCount) --tostring(self.currentAutoCount);
    end
...
function JSYCEntry.AddMultiple()
    ...
    self.FreeMultiple.text = self.ShowText(_count);
end
