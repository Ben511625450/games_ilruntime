...
function FXGZEntry.SetMaxChipCall()
    ...
    self.ChipNum.text = tostring(self.CurrentChip * FXGZ_DataConfig.ALLLINECOUNT);
end
...
function FXGZEntry.Roll()
    ...
    if self.isFreeGame then
        self.WinNum.text = tostring(FXGZ_Result.totalFreeGold);
    else
...
function FXGZEntry.InitPanel()
    ...
    for i = 1, self.SceneData.chipNum do
        ...
        num.text = self.GetCustomK(self.SceneData.chipList[i] * FXGZ_DataConfig.ALLLINECOUNT);
