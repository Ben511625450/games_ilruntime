...
function XYSGJEntry.SetAutoNum(num)
    self.AutoStartBtn.transform:Find("Text"):GetComponent("TextMeshProUGUI").text = self.ShowText(num)
end
...
function XYSGJEntry.InitPanel()
    ...
    for i = 1, #self.SceneData.chipList do
        local newStr = self.SetKM(self.SceneData.chipList[i] * XYSGJ_DataConfig.ALLLINECOUNT)
        self.YZmainPanel:GetChild(i - 1):GetChild(0):GetComponent("TextMeshProUGUI").text = self.ShowText(newStr)
    ...
    self.CurrentChip = XYSGJEntry.SceneData.bet;
    self.ChipNum.text = self.ShowText(XYSGJEntry.SceneData.bet * XYSGJ_DataConfig.ALLLINECOUNT)
