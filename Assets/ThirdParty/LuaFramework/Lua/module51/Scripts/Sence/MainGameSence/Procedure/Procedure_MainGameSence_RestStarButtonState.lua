...
function Procedure_MainGameSence_RestStarButtonState:OnRuning()
    ...
    if (LogicDataSpace.isRoll == true) then
        if (LogicDataSpace.FreeGameCount > 0) then
            ...
            sence.SenceHost.transform:Find("DownInfo/StarBtn/AutoNum"):GetComponent("Text").text = "免费次数" .. LogicDataSpace.FreeGameCount .. "次";
            return
        end
        ...
        if (LogicDataSpace.SeleteAutoNumber > 0) then
            sence.SenceHost.transform:Find("DownInfo/StarBtn/AutoNum"):GetComponent("Text").text = "自动次数" .. LogicDataSpace.SeleteAutoNumber .. "次";
        end
        if (LogicDataSpace.SeleteAutoNumber < 0) then            
            sence.SenceHost.transform:Find("DownInfo/StarBtn/AutoNum"):GetComponent("Text").text = "无限";
        end
        return
    end
    if (LogicDataSpace.FreeGameCount > 0) then
