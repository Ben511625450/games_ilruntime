...
public class AspectRatioController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (!Screen.fullScreen && setWidth != -1 && setHeight != -1 )
        {
            if (Screen.height != setHeight)
            {
            ...
            {
                //根据宽度设置宽度，因为Aero Snap不会触发WM_SIZING。
                setWidth = Screen.width;
                setHeight = Mathf.RoundToInt(Screen.width / aspect);
            }
