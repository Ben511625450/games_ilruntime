...
namespace LuaFramework
{
    public class GameManager : Manager
    {
        ...
        private void InitILRuntime()
        {
            GameObject go;
#if UNITY_EDITOR
            go = UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>($"Assets/Prefab/Dll.prefab");

#else
            go = Util.LoadAsset("module02/Pool/dll", "Dll");
#endif
            if (go == null)
            {
            ...
            byte[] pdb = go.GetComponent<ReferenceCollector>().Get<TextAsset>($"Hotfix.pdb").bytes;
            AppFacade.Instance.GetManager<ILRuntimeManager>().LoadHotFix(dll, pdb);
        }
    }
}